...
set(Qt5WaylandClient_VERSION_STRING 5.15.16)
...
function(_qt5_WaylandClient_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5WaylandClient_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5WaylandClient_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5WaylandClient_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5WaylandClient_${Configuration}_${_lib}_PATH)
                    if(_Qt5WaylandClient_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5WaylandClient_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_WaylandClient_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5WaylandClient_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5WaylandClient_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::WaylandClient APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::WaylandClient)
    ...
    set(Qt5WaylandClient_PRIVATE_INCLUDE_DIRS
        "${_qt5WaylandClient_install_prefix}/include/QtWaylandClient/5.15.16"
        "${_qt5WaylandClient_install_prefix}/include/QtWaylandClient/5.15.16/QtWaylandClient"
    )
    ...
    foreach(_module_dep ${_Qt5WaylandClient_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.16 ${_Qt5WaylandClient_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_WaylandClient_process_prl_file(
            "${_qt5WaylandClient_install_prefix}/lib/libQt5WaylandClient.prl" RELEASE
            _Qt5WaylandClient_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5WaylandClient_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::WaylandClient STATIC IMPORTED)
    set_property(TARGET Qt5::WaylandClient PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    set_property(TARGET Qt5::WaylandClient
    ...
    set_property(TARGET Qt5::WaylandClient
                 ...
                 wayland-client;egl-extension-platform-wayland;wayland-client-fullscreen-shell-v1;wayland-client-ivi-shell;wayland-client-primary-selection;wayland-client-wl-shell;wayland-client-xdg-shell;wayland-client-xdg-shell-v5;wayland-client-xdg-shell-v6;wayland-datadevice;wayland-drm-egl-server-buffer;wayland-egl;wayland-shm-emulation-server-buffer;xcomposite-glx)
    set_property(TARGET Qt5::WaylandClient
                 PROPERTY QT_DISABLED_PRIVATE_FEATURES
                 wayland-brcm;wayland-dmabuf-server-buffer;wayland-libhybris-egl-server-buffer;wayland-vulkan-server-buffer;xcomposite-egl)
    ...
    if (_Qt5WaylandClient_PRIVATE_DIRS_EXIST)
        ...
        if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND NOT TARGET Qt::WaylandClientPrivate)
            add_library(Qt::WaylandClientPrivate INTERFACE IMPORTED)
            set_target_properties(Qt::WaylandClientPrivate PROPERTIES
                INTERFACE_LINK_LIBRARIES "Qt5::WaylandClientPrivate"
            )
        endif()
    endif()

    _populate_WaylandClient_target_properties(RELEASE "libQt5WaylandClient.a" "" FALSE)
    ...
    if (QT5_STRICT_PLUGIN_GLOB OR Qt5WaylandClient_STRICT_PLUGIN_GLOB)
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5WaylandClient_*Plugin.cmake")
    else()
        file(GLOB pluginTargets "${CMAKE_CURRENT_LIST_DIR}/Qt5WaylandClient_*.cmake")
    endif()
...
# Add a versionless target, for compatibility with Qt6.
if(NOT "${QT_NO_CREATE_VERSIONLESS_TARGETS}" AND TARGET Qt5::WaylandClient AND NOT TARGET Qt::WaylandClient)
    add_library(Qt::WaylandClient INTERFACE IMPORTED)
    set_target_properties(Qt::WaylandClient PROPERTIES
        INTERFACE_LINK_LIBRARIES "Qt5::WaylandClient"
    )
endif()
