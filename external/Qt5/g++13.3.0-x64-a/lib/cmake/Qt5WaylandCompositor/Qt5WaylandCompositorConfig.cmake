...
set(Qt5WaylandCompositor_VERSION_STRING 5.15.16)
...
function(_qt5_WaylandCompositor_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5WaylandCompositor_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5WaylandCompositor_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5WaylandCompositor_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5WaylandCompositor_${Configuration}_${_lib}_PATH)
                    if(_Qt5WaylandCompositor_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5WaylandCompositor_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_WaylandCompositor_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5WaylandCompositor_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5WaylandCompositor_STATIC_${Configuration}_LINK_FLAGS}")
    if(_static_link_flags)
        set(_static_link_flags_genex "$<${_genex_condition}:${_static_link_flags}>")
        if(NOT CMAKE_VERSION VERSION_LESS "3.13")
            set_property(TARGET Qt5::WaylandCompositor APPEND PROPERTY INTERFACE_LINK_OPTIONS
...
if (NOT TARGET Qt5::WaylandCompositor)
    ...
    set(Qt5WaylandCompositor_PRIVATE_INCLUDE_DIRS
        "${_qt5WaylandCompositor_install_prefix}/include/QtWaylandCompositor/5.15.16"
        "${_qt5WaylandCompositor_install_prefix}/include/QtWaylandCompositor/5.15.16/QtWaylandCompositor"
    )
    ...
    foreach(_module_dep ${_Qt5WaylandCompositor_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.16 ${_Qt5WaylandCompositor_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_WaylandCompositor_process_prl_file(
            "${_qt5WaylandCompositor_install_prefix}/lib/libQt5WaylandCompositor.prl" RELEASE
            _Qt5WaylandCompositor_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5WaylandCompositor_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::WaylandCompositor STATIC IMPORTED)
    set_property(TARGET Qt5::WaylandCompositor PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    set_property(TARGET Qt5::WaylandCompositor
                 ...
                 wayland-server;wayland-datadevice;wayland-drm-egl-server-buffer;wayland-egl;wayland-shm-emulation-server-buffer;xcomposite-egl;xcomposite-glx)
    set_property(TARGET Qt5::WaylandCompositor
                 PROPERTY QT_DISABLED_PRIVATE_FEATURES
                 wayland-brcm;wayland-dmabuf-client-buffer;wayland-dmabuf-server-buffer;wayland-layer-integration-vsp2;wayland-libhybris-egl-server-buffer;wayland-vulkan-server-buffer)
    ...
    _populate_WaylandCompositor_target_properties(RELEASE "libQt5WaylandCompositor.a" "" FALSE)
