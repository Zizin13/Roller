...
set(Qt5Quick_VERSION_STRING 5.15.16)
...
function(_qt5_Quick_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5Quick_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5Quick_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5Quick_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5Quick_${Configuration}_${_lib}_PATH)
                    if(_Qt5Quick_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5Quick_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_Quick_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5Quick_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5Quick_STATIC_${Configuration}_LINK_FLAGS}")
...
if (NOT TARGET Qt5::Quick)
    ...
    set(Qt5Quick_PRIVATE_INCLUDE_DIRS
        "${_qt5Quick_install_prefix}/include/QtQuick/5.15.16"
        "${_qt5Quick_install_prefix}/include/QtQuick/5.15.16/QtQuick"
    )
    ...
    foreach(_module_dep ${_Qt5Quick_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.16 ${_Qt5Quick_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_Quick_process_prl_file(
            "${_qt5Quick_install_prefix}/lib/libQt5Quick.prl" RELEASE
            _Qt5Quick_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5Quick_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::Quick STATIC IMPORTED)
    set_property(TARGET Qt5::Quick PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_Quick_target_properties(RELEASE "libQt5Quick.a" "" FALSE)
