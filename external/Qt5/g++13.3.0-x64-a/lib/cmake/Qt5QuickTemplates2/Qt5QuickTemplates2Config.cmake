...
set(Qt5QuickTemplates2_VERSION_STRING 5.15.16)
...
function(_qt5_QuickTemplates2_process_prl_file prl_file_location Configuration lib_deps link_flags)
    set(_lib_deps)
    set(_link_flags)

    set(_qt5_install_libs "${_qt5QuickTemplates2_install_prefix}/lib/")
            ...
            elseif(_framework_flag OR _flag MATCHES "^-l(.*)$")
                ...
                elseif(_lib MATCHES "^pthread$")
                    ...
                    if(current_search_paths)
                        find_library(_Qt5QuickTemplates2_${Configuration}_${_lib}_PATH ${_lib} HINTS ${current_search_paths} NO_DEFAULT_PATH)
                    endif()
                    find_library(_Qt5QuickTemplates2_${Configuration}_${_lib}_PATH ${_lib} HINTS ${CMAKE_CXX_IMPLICIT_LINK_DIRECTORIES})
                    mark_as_advanced(_Qt5QuickTemplates2_${Configuration}_${_lib}_PATH)
                    if(_Qt5QuickTemplates2_${Configuration}_${_lib}_PATH)
                        list(APPEND _lib_deps
                            ${_Qt5QuickTemplates2_${Configuration}_${_lib}_PATH}
                        )
                    else()
...
macro(_populate_QuickTemplates2_target_properties Configuration LIB_LOCATION IMPLIB_LOCATION
    ...
    set(_static_deps
        ${_Qt5QuickTemplates2_STATIC_${Configuration}_LIB_DEPENDENCIES}
    )
    ...
    set(_static_link_flags "${_Qt5QuickTemplates2_STATIC_${Configuration}_LINK_FLAGS}")
...
if (NOT TARGET Qt5::QuickTemplates2)
    ...
    set(Qt5QuickTemplates2_PRIVATE_INCLUDE_DIRS
        "${_qt5QuickTemplates2_install_prefix}/include/QtQuickTemplates2/5.15.16"
        "${_qt5QuickTemplates2_install_prefix}/include/QtQuickTemplates2/5.15.16/QtQuickTemplates2"
    )
    ...
    foreach(_module_dep ${_Qt5QuickTemplates2_MODULE_DEPENDENCIES})
        if (NOT Qt5${_module_dep}_FOUND)
            find_package(Qt5${_module_dep}
                5.15.16 ${_Qt5QuickTemplates2_FIND_VERSION_EXACT}
    ...
    if(NOT Qt5_EXCLUDE_STATIC_DEPENDENCIES)
        ...
        _qt5_QuickTemplates2_process_prl_file(
            "${_qt5QuickTemplates2_install_prefix}/lib/libQt5QuickTemplates2.prl" RELEASE
            _Qt5QuickTemplates2_STATIC_RELEASE_LIB_DEPENDENCIES
            _Qt5QuickTemplates2_STATIC_RELEASE_LINK_FLAGS
        )
    endif()

    add_library(Qt5::QuickTemplates2 STATIC IMPORTED)
    set_property(TARGET Qt5::QuickTemplates2 PROPERTY IMPORTED_LINK_INTERFACE_LANGUAGES CXX)
    ...
    _populate_QuickTemplates2_target_properties(RELEASE "libQt5QuickTemplates2.a" "" FALSE)
